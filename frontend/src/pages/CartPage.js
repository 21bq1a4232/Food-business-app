...
import { useCart } from '../App'; // Import your real useCart hook
...
const CartPage = () => {
  const { cartItems, cartTotal, updateCartItem, changeCartItemWeight } = useCart(); // Use real cart hook
  const navigate = useNavigate(); // Use real navigate hook
  ...
  const handleWeightChange = (item, newWeightOption) => {
    ...
    if (changeCartItemWeight) {
      changeCartItemWeight(item.product_id, item.selected_weight, newWeightOption);
    } else {
          ...
          {cartItems.map((item, index) => (
            ...
            >
              <div className="flex items-center gap-4">
                ...
                <div className="w-20 h-20 bg-gradient-to-br from-orange-100 to-red-100 rounded-xl flex items-center justify-center flex-shrink-0">
                  <span className="text-2xl">
                    {item.name && item.name.includes('Pickle') && '🥭'}
                    {item.name && item.name.includes('Jamun') && '🍮'}
                    {item.name && item.name.includes('Chole') && '🍛'}
                    {item.name && item.name.includes('Samosa') && '🥟'}
                    {item.name && item.name.includes('Dal') && '🍛'}
                    {item.name && item.name.includes('Katli') && '🍮'}
                    {(!item.name || !item.name.match(/(Pickle|Jamun|Chole|Samosa|Dal|Katli)/)) && '🍽️'}
                  </span>
                </div>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-orange-600 font-semibold">
                    ₹{item.price} per {item.selected_weight || item.weight}
                  </p>
                ...
                <div className="flex items-center gap-3 bg-gray-50 rounded-lg p-2">
                  <button
                    onClick={() => handleQuantityUpdate(
                      item.product_id, 
                      item.quantity - 1, 
                      item.selected_weight || item.weight
                    )}
